<template>
  <section>
    	<div id="slider-toolbar">
          <div class="pull-left">
							<h4>滚动图管理</h4>
							<router-link to="/Admin/slider/addslider" exact  class="waves-effect waves-light btn"><i class="fa fa-plus" aria-hidden="true"></i>添加</router-link>
							<!--<router-link to="/Admin/column"  class="waves-effect waves-light btn del"><i class="fa fa-minus" aria-hidden="true"></i>添加</router-link>-->
          </div>
      </div>
	  	<table id="slider-table"  class="responsive-table"></table>
  </section>
</template>
...
<script>
    ...
    oTableInit.Init = function () {
        $table.bootstrapTable({
            url: '/Api/slider',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            toolbar: '#slider-toolbar',                //工具按钮用哪个容器
    ...
    function onEditableSave(value, row, index){
      $.post('/Api/editslider',{id:row.id,title:row.title},
			function(data){},"json");
    }
    //在table表里写dom结构
    function imgFormatter(value, row, index) {
	    	return [
						'<img class="materialboxed" width="100" src="../'+row.img+'" />',
	    	].join('');
    }
    ...
    window.operateEvents = {
       'click .remove': function (e, value, row, index) {
           if (confirm('确定删除?')==true){
						$.post('/Api/delslider',{id:row.id,file:row.img},
							function(data){
								if(data.status==1){
    ...
    window.openEvents = {
        'click .lever': function (e, value, row, index) {
	        ...
	        if(ch.is(':checked')) {
   	      ...
   	      $.post('/Api/openslider',{id:row.id,open:open},
				  function(data){},"json");
        }
    };
    return oTableInit;
